...
color4 vertex_colors[8] = {
    ...
    color4( 0.7, 0.0, 0.0, 1.0 ),  // red
    color4( 1.0, 1.0, 0.0, 1.0 ),  // yellow
    color4( 0.0, 1.0, 0.0, 1.0 ),  // green
    color4( 0.0, 0.0, 1.0, 1.0 ),  // blue
    color4( 1.0, 0.0, 1.0, 1.0 ),  // magenta
    color4( 1.0, 1.0, 1.0, 1.0 ),  // white
    color4( 0.0, 0.8, 0.8, 0.9 )   // cyan
};
...
const GLfloat LOWER_ARM_HEIGHT = 12.5;
const GLfloat LOWER_ARM_WIDTH  = 0.5;
const GLfloat UPPER_ARM_HEIGHT = 12.5;
...
int      Axis = Base;
GLfloat  Theta[NumAngles] = { 0.0, 0.0, 0.0 };
...
quad( int a, int b, int c, int d )
{
	 ...
	 vec3  normal2 = normalize( cross(vertices[c] - vertices[b], vertices[d] - vertices[c]) );

    colors[Index] = vertex_colors[7]; points[Index] = vertices[a]; normals[Index] = normal2; Index++;
    colors[Index] = vertex_colors[7]; points[Index] = vertices[b]; normals[Index] = normal2; Index++;
    colors[Index] = vertex_colors[7]; points[Index] = vertices[c]; normals[Index] = normal2; Index++;
    colors[Index] = vertex_colors[7]; points[Index] = vertices[a]; normals[Index] = normal2; Index++;
    colors[Index] = vertex_colors[7]; points[Index] = vertices[c]; normals[Index] = normal2; Index++;
    colors[Index] = vertex_colors[7]; points[Index] = vertices[d]; normals[Index] = normal2; Index++;
}
...
void base() {
    ...
    glUniformMatrix4fv( MV_loc, 1, GL_TRUE, model_view * instance );
...
//-------------------------
vec2 findAngles (vec2 point) {
	float arm = LOWER_ARM_HEIGHT; // both arms will be equal
	float length = sqrt(pow(point.x - armOrigin.x, 2)+pow(point.y - armOrigin.y, 2));
	//cout<<"length is:"<<length<<endl;
	//now we have 3 lines that form a triangle, time to find the angles we need.

	float a = acos((length/2)/arm) *180.0 / PI;
	float b = acos( (point.x-armOrigin.x)/length) * 180.0 / PI;

	float theta_base = a+b;
	float theta_between = (180-2*a);
	//cout<<"Angles are: " << theta_base<<" | "<<theta_between<<endl;
	return vec2((270 + theta_base), (180+theta_between));
