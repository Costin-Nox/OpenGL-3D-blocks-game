...
bool collisionCheck()
{
	...
	for (int i = 0; i<4;i++)
	{
		t_x = tilepos.x + allRotationShape[rotation][i].x;
		t_y = tilepos.y + allRotationShape[rotation][i].y;
...
void newtile()
{
	...
	released = false;
	tilepos = startPosition; // Put the tile at the top of the board
...
void init()
{
	...
	// Game initialization
	//place robot hands
	robo_arm::Theta[1] = robo_arm::findAngles(startPosition).x;
	robo_arm::Theta[2] = robo_arm::findAngles(startPosition).y;
